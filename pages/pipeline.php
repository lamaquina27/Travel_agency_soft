...
App::requireLogin();
$user = App::getUser();
if (!in_array($user['role'], ['admin', 'agent'])) { App::redirect('/dashboard'); exit; }
...
function pl_rgb($hex){ $h=ltrim(trim($hex),'#'); if(strlen($h)===3)$h=$h[0].$h[0].$h[1].$h[1].$h[2].$h[2]; if(!preg_match('/^[0-9a-fA-F]{6}$/',$h))return'99,102,241'; return hexdec(substr($h,0,2)).','.hexdec(substr($h,2,2)).','.hexdec(substr($h,4,2)); }
...
.header-user{display:flex;align-items:center;gap:8px;background:rgba(255,255,255,.15);border-radius:22px;padding:6px 14px 6px 6px;cursor:default;}
...
/* TOOLBAR */
.pl-toolbar{
  height:58px;background:#fff;border-bottom:1px solid #e8edf2;
  display:flex;align-items:center;gap:10px;padding:0 20px;
  flex-shrink:0;overflow-x:auto;
}
.pl-toolbar::-webkit-scrollbar{height:3px;}
.pl-toolbar::-webkit-scrollbar-thumb{background:#e2e8f0;border-radius:3px;}

.view-toggle{display:flex;background:#f1f5f9;border-radius:8px;padding:3px;gap:2px;flex-shrink:0;}
.vbtn{padding:5px 13px;border:none;background:transparent;border-radius:6px;font-size:13px;font-weight:500;color:#64748b;cursor:pointer;display:flex;align-items:center;gap:5px;transition:all .18s;}
.vbtn.active{background:#fff;color:var(--pr);box-shadow:0 1px 4px rgba(0,0,0,.1);}
.vbtn svg{width:14px;height:14px;stroke:currentColor;fill:none;stroke-width:2;}

.pl-search{flex:1;min-width:140px;max-width:260px;display:flex;align-items:center;gap:7px;background:#f8fafc;border:1px solid #e2e8f0;border-radius:8px;padding:0 11px;height:36px;}
.pl-search svg{width:14px;height:14px;stroke:#94a3b8;fill:none;stroke-width:2;flex-shrink:0;}
.pl-search input{border:none;background:transparent;outline:none;font-size:13px;color:#1e293b;width:100%;}
.pl-search input::placeholder{color:#94a3b8;}

.pl-sel{height:36px;padding:0 10px;border:1px solid #e2e8f0;border-radius:8px;background:#f8fafc;font-size:13px;color:#475569;outline:none;cursor:pointer;flex-shrink:0;}
.pl-sel:focus{border-color:var(--pr);}

.toolbar-divider{width:1px;height:24px;background:#e8edf2;flex-shrink:0;}

.btn-config{height:36px;width:36px;border:1px solid #e2e8f0;border-radius:8px;background:#f8fafc;cursor:pointer;display:flex;align-items:center;justify-content:center;color:#64748b;transition:all .18s;flex-shrink:0;}
.btn-config:hover{background:#fff;border-color:var(--pr);color:var(--pr);}
.btn-config svg{width:16px;height:16px;stroke:currentColor;fill:none;stroke-width:2;}

.btn-new{height:36px;padding:0 16px;border:none;border-radius:8px;background:var(--grad);color:#fff;font-size:13px;font-weight:600;cursor:pointer;display:flex;align-items:center;gap:6px;transition:opacity .18s;flex-shrink:0;white-space:nowrap;}
.btn-new:hover{opacity:.88;}
.btn-new svg{width:14px;height:14px;stroke:#fff;fill:none;stroke-width:2.5;}

/* BOARD */
.pl-board-wrap{flex:1;overflow:hidden;display:flex;flex-direction:column;}
.board-scroll{flex:1;overflow-x:auto;overflow-y:hidden;padding:18px;}
.board-scroll::-webkit-scrollbar{height:6px;}
.board-scroll::-webkit-scrollbar-track{background:#f1f5f9;border-radius:3px;}
.board-scroll::-webkit-scrollbar-thumb{background:#cbd5e1;border-radius:3px;}
.kanban-board{display:flex;gap:14px;height:100%;align-items:flex-start;min-width:max-content;}

/* COLUMN */
.k-col{width:284px;flex-shrink:0;display:flex;flex-direction:column;background:#f8fafc;border-radius:14px;border:1px solid #e2e8f0;max-height:calc(100vh - 70px - 58px - 36px);}
.k-col-hd{padding:12px 14px 10px;border-radius:14px 14px 0 0;border-top:4px solid var(--cc,#6366f1);background:#fff;display:flex;align-items:center;gap:8px;border-bottom:1px solid #f1f5f9;}
.k-dot{width:8px;height:8px;border-radius:50%;background:var(--cc,#6366f1);flex-shrink:0;}
.k-name{font-size:13px;font-weight:700;color:#0f172a;flex:1;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;}
.k-cnt{background:var(--cc,#6366f1);color:#fff;border-radius:12px;padding:1px 8px;font-size:11px;font-weight:700;flex-shrink:0;}
.k-body{flex:1;overflow-y:auto;padding:8px;display:flex;flex-direction:column;gap:7px;min-height:50px;border-radius:0 0 14px 14px;transition:background .15s;}
.k-body::-webkit-scrollbar{width:3px;}
.k-body::-webkit-scrollbar-thumb{background:#e2e8f0;border-radius:3px;}
.k-body.drag-over{background:rgba(99,102,241,.07);outline:2px dashed rgba(99,102,241,.3);outline-offset:-4px;}
.k-empty{text-align:center;padding:26px 12px;color:#cbd5e1;font-size:12px;}
.k-empty svg{width:28px;height:28px;stroke:#dde3eb;fill:none;stroke-width:1.5;display:block;margin:0 auto 6px;}

/* CARD */
.lead-card{background:#fff;border-radius:10px;padding:13px;cursor:pointer;border:1px solid #f1f5f9;box-shadow:0 1px 4px rgba(0,0,0,.05);transition:box-shadow .18s,transform .15s,opacity .15s;user-select:none;position:relative;}
.lead-card:hover{box-shadow:0 4px 16px rgba(0,0,0,.10);transform:translateY(-2px);border-color:#e2e8f0;}
.lead-card.dragging{opacity:.3;cursor:grabbing;transform:rotate(2deg);}
.c-top{display:flex;align-items:flex-start;justify-content:space-between;gap:6px;margin-bottom:6px;}
.c-name{font-size:13.5px;font-weight:700;color:#0f172a;flex:1;min-width:0;line-height:1.3;}
.c-email-ico{width:18px;height:18px;border-radius:4px;background:#eff6ff;display:flex;align-items:center;justify-content:center;flex-shrink:0;}
.c-email-ico svg{width:10px;height:10px;stroke:#3b82f6;fill:none;stroke-width:2;}
.c-sub{font-size:12px;color:#64748b;display:flex;align-items:center;gap:4px;margin-bottom:3px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;}
.c-sub svg{width:11px;height:11px;stroke:#94a3b8;fill:none;stroke-width:2;flex-shrink:0;}
.c-foot{display:flex;align-items:center;gap:5px;margin-top:10px;flex-wrap:wrap;}
.c-budget{font-size:11.5px;font-weight:600;color:#16a34a;background:#f0fdf4;border:1px solid #bbf7d0;padding:2px 7px;border-radius:10px;}
.c-tag{font-size:11px;font-weight:600;padding:2px 8px;border-radius:10px;}
.c-asesor{margin-left:auto;width:23px;height:23px;border-radius:50%;background:var(--grad);color:#fff;font-size:9px;font-weight:700;display:flex;align-items:center;justify-content:center;}

/* LIST VIEW */
.list-scroll{flex:1;overflow-y:auto;padding:18px;}
.list-tbl{width:100%;border-collapse:separate;border-spacing:0 5px;}
.list-tbl thead th{padding:9px 14px;text-align:left;font-size:11px;font-weight:600;color:#94a3b8;text-transform:uppercase;letter-spacing:.5px;cursor:pointer;white-space:nowrap;}
.list-tbl thead th:hover{color:var(--pr);}
.list-tbl tbody tr{background:#fff;box-shadow:0 1px 4px rgba(0,0,0,.05);border-radius:10px;cursor:pointer;transition:box-shadow .15s,transform .1s;}
.list-tbl tbody tr:hover{box-shadow:0 3px 12px rgba(0,0,0,.10);transform:translateY(-1px);}
.list-tbl tbody td{padding:11px 14px;font-size:13px;color:#334155;}
.list-tbl tbody td:first-child{border-radius:10px 0 0 10px;font-weight:600;color:#0f172a;}
.list-tbl tbody td:last-child{border-radius:0 10px 10px 0;}
.l-badge{display:inline-flex;align-items:center;gap:5px;padding:3px 10px;border-radius:20px;font-size:11.5px;font-weight:600;}
.l-bdot{width:6px;height:6px;border-radius:50%;background:currentColor;}

/* ─── LEAD DETAIL MODAL (2 columns) ─── */
.lm-overlay{position:fixed;inset:0;background:rgba(15,23,42,.55);z-index:1100;display:none;align-items:center;justify-content:center;padding:16px;}
.lm-overlay.open{display:flex;}
.lm-box{background:#fff;border-radius:20px;width:min(960px,94vw);height:min(700px,88vh);display:flex;flex-direction:column;overflow:hidden;box-shadow:0 28px 64px rgba(0,0,0,.22);animation:mdIn .24s cubic-bezier(.4,0,.2,1);}
@keyframes mdIn{from{opacity:0;transform:scale(.94) translateY(18px);}to{opacity:1;transform:scale(1) translateY(0);}}
@keyframes fadeIn{from{opacity:0;transform:translateY(6px);}to{opacity:1;transform:translateY(0);}}

.lm-header{padding:16px 22px;background:var(--grad);border-bottom:none;display:flex;align-items:center;gap:14px;flex-shrink:0;}
.lm-avatar{width:46px;height:46px;border-radius:50%;background:rgba(255,255,255,.22);color:#fff;font-size:16px;font-weight:700;display:flex;align-items:center;justify-content:center;flex-shrink:0;border:2px solid rgba(255,255,255,.35);}
.lm-hinfo{flex:1;min-width:0;}
.lm-hname{font-size:17px;font-weight:700;color:#fff;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;}
.lm-hemail{font-size:12px;color:rgba(255,255,255,.75);margin-top:2px;}
.lm-hbadge{display:inline-flex;align-items:center;gap:5px;padding:4px 12px;border-radius:20px;font-size:12px;font-weight:600;flex-shrink:0;background:rgba(255,255,255,.2);color:#fff;}
.lm-close{width:32px;height:32px;border:none;background:rgba(255,255,255,.18);border-radius:8px;cursor:pointer;display:flex;align-items:center;justify-content:center;color:#fff;transition:all .18s;flex-shrink:0;}
.lm-close:hover{background:rgba(255,255,255,.3);}
.lm-close svg{width:16px;height:16px;stroke:currentColor;fill:none;stroke-width:2.2;}

.lm-body{display:flex;flex:1;overflow:hidden;}

/* Left info panel */
/* Left panel — info-group style (matching chat.php sidebar) */
.lm-left{width:320px;flex-shrink:0;border-right:1px solid #e2e8f0;overflow-y:auto;display:flex;flex-direction:column;background:#fff;}
.lm-left::-webkit-scrollbar{width:3px;}
.lm-left::-webkit-scrollbar-thumb{background:#e2e8f0;border-radius:3px;}
.lm-ig{display:flex;align-items:flex-start;gap:12px;padding:10px 16px;transition:background .2s;}
.lm-ig:hover{background:#f8fafc;}
.lm-ig-icon{width:34px;height:34px;border-radius:9px;display:flex;align-items:center;justify-content:center;flex-shrink:0;color:#94a3b8;}
.lm-ig-icon svg{width:18px;height:18px;stroke:currentColor;fill:none;stroke-width:2;stroke-linecap:round;stroke-linejoin:round;}
.lm-ig-content{flex:1;min-width:0;}
.lm-ig-label{font-size:11px;text-transform:uppercase;color:#94a3b8;font-weight:600;letter-spacing:.05em;display:block;margin-bottom:3px;}
.lm-ig-val{font-size:14px;color:#1e293b;font-weight:500;display:block;word-break:break-word;line-height:1.4;}
.lm-ig-val.dim{color:#cbd5e1;font-weight:400;}
.lm-ig-divider{height:1px;background:#f1f5f9;margin:4px 16px;}
.lm-ig-select{font-size:14px;color:#1e293b;font-weight:500;background:transparent;border:none;border-bottom:1.5px solid transparent;padding:4px;margin-left:-4px;width:calc(100% + 8px);outline:none;font-family:inherit;cursor:pointer;-webkit-appearance:none;appearance:none;transition:all .2s;}
.lm-ig-select:hover{background:#f1f5f9;border-radius:4px;}
.lm-ig-select:focus{border-bottom-color:var(--pr);}
.lm-ig-tags{display:flex;flex-wrap:wrap;gap:5px;margin-top:3px;}
.lm-vinc-btn{display:flex;align-items:center;gap:8px;padding:9px 14px;border:1.5px solid #e2e8f0;border-radius:10px;background:#fff;font-size:13px;color:#475569;font-weight:600;cursor:pointer;width:100%;transition:all .18s;}
.lm-vinc-btn:hover{border-color:var(--pr);color:var(--pr);background:rgba(var(--pr-rgb),.04);}
.lm-vinc-btn svg{width:14px;height:14px;stroke:currentColor;fill:none;stroke-width:2;flex-shrink:0;}
.lm-vinc-badge{font-size:11.5px;color:#16a34a;background:#f0fdf4;border:1px solid #bbf7d0;padding:3px 8px;border-radius:8px;display:none;margin-top:6px;}
.lm-vinc-badge.show{display:block;}

/* Right conversation panel — Chat completo (estilo chat.php) */
.lm-right{flex:1;display:flex;flex-direction:column;overflow:hidden;background:#fafaf9;}
.lm-chat-hd{padding:20px 30px;background:#fff;border-bottom:1px solid #e2e8f0;flex-shrink:0;}
.lm-chat-hd-title{font-size:18px;font-weight:600;color:#0f172a;margin:0 0 3px;}
.lm-chat-hd-sub{display:flex;align-items:center;gap:8px;font-size:13px;color:#64748b;}
.lm-gmail-badge{font-size:11px;font-weight:600;padding:2px 8px;border-radius:20px;flex-shrink:0;}
.lm-gmail-badge.ok{background:#dcfce7;color:#15803d;}
.lm-gmail-badge.nok{background:#fee2e2;color:#dc2626;}
.lm-chat-msgs{flex:1;overflow-y:auto;padding:30px;display:flex;flex-direction:column;gap:16px;}
.lm-chat-msgs::-webkit-scrollbar{width:4px;}
.lm-chat-msgs::-webkit-scrollbar-thumb{background:#e2e8f0;border-radius:4px;}
.lm-msg{max-width:70%;padding:16px;border-radius:16px;font-size:15px;line-height:1.5;word-break:break-word;animation:fadeIn .3s ease-out;}
...
.lm-no-gmail{display:none;padding:9px 16px;background:#fffbeb;border-top:1px solid #fde68a;font-size:12px;color:#92400e;text-align:center;flex-shrink:0;}
.lm-no-gmail a{color:#b45309;font-weight:700;text-decoration:underline;}
.lm-no-gmail.show{display:block;}
.lm-composer{background:#fff;border-top:1px solid #e2e8f0;display:flex;flex-direction:column;flex-shrink:0;}
.lm-fmt-bar{display:flex;align-items:center;gap:2px;padding:8px 16px;border-bottom:1px solid #f1f5f9;background:#f8fafc;}
.lm-fmt-btn{width:30px;height:28px;border:none;background:transparent;border-radius:5px;cursor:pointer;color:#64748b;font-size:13px;font-family:'Georgia',serif;display:flex;align-items:center;justify-content:center;transition:background .15s,color .15s;flex-shrink:0;}
.lm-fmt-btn:hover{background:#e2e8f0;color:#0f172a;}
.lm-fmt-btn.active{background:rgba(var(--pr-rgb),.12);color:var(--pr);}
.lm-fmt-divider{width:1px;height:18px;background:#e2e8f0;margin:0 4px;flex-shrink:0;}
.lm-editor-wrap{padding:12px 16px;}
.lm-editor{width:100%;min-height:70px;max-height:150px;border:none;outline:none;font-family:inherit;font-size:14px;color:#1e293b;background:transparent;line-height:1.6;overflow-y:auto;word-break:break-word;white-space:pre-wrap;}
.lm-editor:empty::before{content:attr(data-placeholder);color:#cbd5e1;pointer-events:none;}
.lm-attachments{display:none;flex-wrap:wrap;gap:6px;padding:0 16px 8px;}
.lm-attachments.has-files{display:flex;}
.lm-att-chip{display:inline-flex;align-items:center;gap:6px;background:#f1f5f9;border:1px solid #e2e8f0;border-radius:6px;padding:4px 10px;font-size:12px;color:#475569;font-weight:500;max-width:200px;}
.lm-att-chip span{overflow:hidden;text-overflow:ellipsis;white-space:nowrap;}
.lm-att-chip svg{width:13px;height:13px;flex-shrink:0;stroke:#64748b;fill:none;}
.lm-att-rm{border:none;background:transparent;cursor:pointer;color:#94a3b8;padding:0;font-size:15px;line-height:1;transition:color .15s;}
.lm-att-rm:hover{color:#ef4444;}
.lm-action-bar{display:flex;align-items:center;justify-content:space-between;padding:8px 12px 10px;}
.lm-act-left{display:flex;align-items:center;gap:4px;}
.lm-act-right{display:flex;align-items:center;gap:8px;}
.lm-act-btn{width:34px;height:34px;border:none;background:transparent;border-radius:7px;cursor:pointer;color:#94a3b8;display:flex;align-items:center;justify-content:center;transition:background .15s,color .15s;flex-shrink:0;position:relative;}
.lm-act-btn:hover{background:#f1f5f9;color:#475569;}
.lm-act-btn svg{width:18px;height:18px;stroke-width:1.8;stroke:currentColor;fill:none;}
.lm-act-div{width:1px;height:20px;background:#e2e8f0;margin:0 4px;}
.lm-send-btn{background:var(--grad);color:#fff;border:none;padding:0 20px;height:36px;border-radius:8px;font-weight:600;font-size:14px;cursor:pointer;display:flex;align-items:center;gap:7px;transition:filter .2s,transform .1s,box-shadow .2s;font-family:inherit;white-space:nowrap;}
.lm-send-btn:hover:not(:disabled){filter:brightness(.95);box-shadow:0 4px 12px rgba(var(--pr-rgb),.3);}
.lm-send-btn:active:not(:disabled){transform:scale(.97);}
.lm-send-btn:disabled{opacity:.55;cursor:not-allowed;}
.lm-send-btn svg{width:15px;height:15px;stroke:#fff;fill:none;stroke-width:2.5;}
.lm-tpl-overlay{position:fixed;inset:0;background:rgba(15,23,42,.55);backdrop-filter:blur(3px);z-index:1300;display:none;align-items:flex-start;justify-content:center;padding-top:80px;}
.lm-tpl-overlay.active{display:flex;}
.lm-tpl-modal{background:#fff;border-radius:14px;width:min(860px,94vw);max-height:75vh;display:flex;flex-direction:column;box-shadow:0 24px 60px rgba(15,23,42,.25);overflow:hidden;animation:mdIn .22s cubic-bezier(.4,0,.2,1);}
.lm-tpl-hd{display:flex;align-items:center;gap:14px;padding:20px 24px;border-bottom:1px solid #f1f5f9;flex-shrink:0;}
.lm-tpl-hd h2{font-size:22px;font-weight:800;color:#1e293b;margin:0;white-space:nowrap;font-family:inherit;}
.lm-tpl-search{flex:1;background:#f4f5f7;border:1px solid transparent;border-radius:8px;padding:11px 14px;font-size:14px;color:#1e293b;outline:none;font-family:inherit;transition:border-color .15s,background .15s;}
.lm-tpl-search:focus{background:#fff;border-color:#cbd5e1;}
.lm-tpl-create-btn{background:var(--grad);color:#fff;border:none;padding:0 18px;height:40px;border-radius:8px;font-weight:600;font-size:13px;cursor:pointer;white-space:nowrap;font-family:inherit;}
.lm-tpl-close{width:34px;height:34px;border:none;background:#f8fafc;border-radius:8px;cursor:pointer;font-size:20px;color:#64748b;display:flex;align-items:center;justify-content:center;transition:all .15s;flex-shrink:0;line-height:1;}
.lm-tpl-close:hover{background:#fee2e2;color:#ef4444;}
.lm-tpl-list{flex:1;overflow-y:auto;padding:8px 12px;}
.lm-tpl-row{display:flex;align-items:center;justify-content:space-between;gap:10px;padding:12px 16px;border-radius:8px;font-size:14px;color:#334155;font-weight:500;transition:background .15s;border-bottom:1px solid #f8fafc;}
.lm-tpl-row:hover{background:#f8fafc;}
.lm-tpl-row-name{flex:1;cursor:pointer;padding:2px 0;}
.lm-tpl-row-name:hover{color:var(--pr);}
.lm-tpl-row-actions{display:flex;gap:4px;flex-shrink:0;}
.lm-tpl-icon-btn{width:28px;height:28px;border:none;background:transparent;border-radius:6px;cursor:pointer;display:flex;align-items:center;justify-content:center;color:#94a3b8;transition:background .15s,color .15s;}
.lm-tpl-icon-btn:hover.edit{background:#eff6ff;color:#3b82f6;}
.lm-tpl-icon-btn:hover.del{background:#fee2e2;color:#ef4444;}
.lm-tpl-icon-btn svg{width:14px;height:14px;stroke:currentColor;fill:none;stroke-width:2;}
.lm-tpl-empty{padding:36px;text-align:center;color:#94a3b8;font-size:14px;}
.lm-tpl-form{display:none;flex-direction:column;gap:10px;padding:16px 24px;border-top:1px solid #f1f5f9;background:#fafbfc;flex-shrink:0;}
.lm-tpl-form-title{font-size:13px;font-weight:700;color:#475569;text-transform:uppercase;letter-spacing:.4px;}
.lm-tpl-form input,.lm-tpl-form textarea{padding:11px 14px;border:1px solid #e2e8f0;border-radius:8px;font-size:13px;color:#1e293b;outline:none;font-family:inherit;background:#fff;transition:border-color .15s;}
.lm-tpl-form input:focus,.lm-tpl-form textarea:focus{border-color:var(--pr);}
.lm-tpl-form textarea{min-height:90px;resize:vertical;}
.lm-tpl-form-actions{display:flex;gap:8px;justify-content:flex-end;}
.lm-tpl-save-btn{background:var(--grad);color:#fff;border:none;padding:10px 22px;border-radius:8px;font-weight:600;font-size:13px;cursor:pointer;font-family:inherit;}
.lm-tpl-cancel-btn{background:#f1f5f9;color:#475569;border:none;padding:10px 18px;border-radius:8px;font-weight:600;font-size:13px;cursor:pointer;font-family:inherit;}

/* ─── NEW LEAD MODAL ─── */
.nl-overlay{position:fixed;inset:0;background:rgba(15,23,42,.55);z-index:1100;display:none;align-items:center;justify-content:center;padding:16px;}
.nl-overlay.open{display:flex;}
.nl-box{background:#fff;border-radius:18px;width:min(560px,96vw);max-height:88vh;display:flex;flex-direction:column;overflow:hidden;box-shadow:0 24px 60px rgba(0,0,0,.2);animation:mdIn .24s cubic-bezier(.4,0,.2,1);}
.nl-hd{padding:18px 22px 14px;border-bottom:1px solid #f1f5f9;display:flex;align-items:center;justify-content:space-between;flex-shrink:0;}
.nl-title{font-size:17px;font-weight:700;color:#0f172a;}
.nl-close{width:32px;height:32px;border:none;background:#f8fafc;border-radius:8px;cursor:pointer;display:flex;align-items:center;justify-content:center;color:#64748b;transition:all .18s;}
.nl-close:hover{background:#fee2e2;color:#ef4444;}
.nl-close svg{width:15px;height:15px;stroke:currentColor;fill:none;stroke-width:2;}
.nl-body{padding:18px 22px;overflow-y:auto;flex:1;}
.nl-body::-webkit-scrollbar{width:4px;}
.nl-body::-webkit-scrollbar-thumb{background:#e2e8f0;border-radius:4px;}
.f-grid{display:grid;grid-template-columns:1fr 1fr;gap:13px;}
.f-field{display:flex;flex-direction:column;gap:5px;}
.f-field.full{grid-column:1/-1;}
.f-lbl{font-size:11px;font-weight:700;color:#475569;text-transform:uppercase;letter-spacing:.4px;}
.f-inp,.f-sel,.f-ta{height:38px;padding:0 12px;border:1px solid #e2e8f0;border-radius:9px;font-size:13.5px;color:#1e293b;background:#f8fafc;outline:none;font-family:inherit;transition:border-color .18s,background .18s;width:100%;}
.f-inp:focus,.f-sel:focus,.f-ta:focus{border-color:var(--pr);background:#fff;box-shadow:0 0 0 3px rgba(var(--pr-rgb),.09);}
.f-ta{height:66px;padding:10px 12px;resize:none;}
/* intlTelInput override */
.iti{width:100%;}
.iti input{width:100% !important;}
/* Tag chips in modal */
.tag-chips{display:flex;flex-wrap:wrap;gap:6px;min-height:36px;align-items:center;}
.tag-chip{padding:4px 12px;border-radius:20px;font-size:12px;font-weight:600;cursor:pointer;border:2px solid transparent;transition:all .15s;opacity:.7;}
.tag-chip.selected{opacity:1;border-color:rgba(0,0,0,.2);box-shadow:0 2px 6px rgba(0,0,0,.12);}
.tag-chip:hover{opacity:1;transform:scale(1.04);}
.nl-ft{padding:12px 22px 16px;border-top:1px solid #f1f5f9;display:flex;gap:9px;justify-content:flex-end;flex-shrink:0;}
.btn-cancel{height:38px;padding:0 18px;border:1px solid #e2e8f0;border-radius:9px;background:#fff;font-size:13px;font-weight:500;color:#475569;cursor:pointer;transition:background .18s;}
.btn-cancel:hover{background:#f8fafc;}
.btn-submit{height:38px;padding:0 22px;border:none;border-radius:9px;background:var(--grad);color:#fff;font-size:13px;font-weight:600;cursor:pointer;transition:opacity .18s;}
.btn-submit:hover{opacity:.88;}
.btn-submit:disabled{opacity:.6;cursor:not-allowed;}

/* ─── VINCULAR MODAL — opciones ─── */
.vinc-opt{display:flex;align-items:center;gap:14px;padding:13px 15px;border:1.5px solid #e2e8f0;border-radius:12px;background:#fff;cursor:pointer;text-align:left;transition:all .15s;width:100%;}
.vinc-opt:hover{border-color:rgba(var(--pr-rgb),.45);background:rgba(var(--pr-rgb),.03);box-shadow:0 3px 12px rgba(0,0,0,.07);}
.vinc-opt-ico{width:40px;height:40px;border-radius:10px;background:rgba(var(--pr-rgb),.1);display:flex;align-items:center;justify-content:center;flex-shrink:0;}
.vinc-opt-ico svg{width:19px;height:19px;stroke:var(--pr);fill:none;stroke-width:2;stroke-linecap:round;stroke-linejoin:round;}
.vinc-opt-title{font-size:13.5px;font-weight:600;color:#0f172a;line-height:1.3;}
.vinc-opt-desc{font-size:11.5px;color:#64748b;margin-top:2px;}
.vinc-opt-arr{width:15px;height:15px;stroke:#cbd5e1;fill:none;stroke-width:2;stroke-linecap:round;stroke-linejoin:round;margin-left:auto;flex-shrink:0;}
.vinc-clone-row{display:flex;align-items:center;justify-content:space-between;gap:12px;padding:10px 12px;border:1px solid #e2e8f0;border-radius:10px;cursor:pointer;transition:all .15s;margin-bottom:6px;}
.vinc-clone-row:hover{border-color:rgba(var(--pr-rgb),.4);background:rgba(var(--pr-rgb),.03);}
.vinc-clone-name{font-size:13px;font-weight:600;color:#0f172a;}
.vinc-clone-meta{font-size:11.5px;color:#64748b;margin-top:2px;}
/* ─── VINCULAR PICKER (existente / plantilla) ─── */
.vinc-psearch{position:relative;margin-bottom:10px;}
.vinc-psearch svg{position:absolute;left:10px;top:50%;transform:translateY(-50%);width:14px;height:14px;stroke:#94a3b8;fill:none;stroke-width:2;stroke-linecap:round;stroke-linejoin:round;}
.vinc-pinput{width:100%;padding:8px 12px 8px 32px;border:1.5px solid #e2e8f0;border-radius:10px;font-size:13px;outline:none;transition:border-color .2s;}
.vinc-pinput:focus{border-color:rgba(var(--pr-rgb),.6);}
.vinc-pgrid{display:grid;grid-template-columns:1fr 1fr;gap:7px;max-height:220px;overflow-y:auto;}
.vinc-pcard{padding:10px 12px;border:1.5px solid #e2e8f0;border-radius:10px;cursor:pointer;transition:all .15s;position:relative;}
.vinc-pcard:hover{border-color:rgba(var(--pr-rgb),.5);background:rgba(var(--pr-rgb),.03);}
.vinc-pcard.sel{border-color:var(--pr);background:rgba(var(--pr-rgb),.07);}
.vinc-pcard.sel::after{content:'✓';position:absolute;top:7px;right:9px;color:var(--pr);font-weight:700;font-size:12px;}
.vinc-ptitle{font-size:12.5px;font-weight:600;color:#0f172a;margin-bottom:2px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;padding-right:16px;}
.vinc-pmeta{font-size:11px;color:#64748b;line-height:1.3;}
.vinc-pempty{grid-column:1/-1;text-align:center;padding:20px;color:#94a3b8;font-size:13px;}

/* ─── CONFIG MODAL (estados + tags) ─── */
.cfg-overlay{position:fixed;inset:0;background:rgba(15,23,42,.55);z-index:1100;display:none;align-items:center;justify-content:center;padding:16px;}
.cfg-overlay.open{display:flex;}
.cfg-box{background:#fff;border-radius:18px;width:min(540px,96vw);max-height:88vh;display:flex;flex-direction:column;overflow:hidden;box-shadow:0 24px 60px rgba(0,0,0,.2);animation:mdIn .24s cubic-bezier(.4,0,.2,1);}
.cfg-hd{padding:18px 22px 14px;border-bottom:1px solid #f1f5f9;display:flex;align-items:center;justify-content:space-between;flex-shrink:0;}
.cfg-title{font-size:17px;font-weight:700;color:#0f172a;}
.cfg-body{overflow-y:auto;flex:1;padding:18px 22px;}
.cfg-body::-webkit-scrollbar{width:4px;}
.cfg-body::-webkit-scrollbar-thumb{background:#e2e8f0;border-radius:4px;}
.cfg-sec-title{font-size:11px;font-weight:700;color:#94a3b8;text-transform:uppercase;letter-spacing:.6px;margin-bottom:12px;margin-top:20px;}
.cfg-sec-title:first-child{margin-top:0;}

/* Estado rows */
.est-list{display:flex;flex-direction:column;gap:7px;}
.est-row{display:flex;align-items:center;gap:9px;background:#f8fafc;border:1px solid #e8edf2;border-radius:10px;padding:9px 12px;transition:border-color .15s;}
.est-row:hover{border-color:var(--pr);}
.est-drag{cursor:grab;color:#cbd5e1;font-size:16px;flex-shrink:0;line-height:1;}
.est-color{width:32px;height:32px;border-radius:7px;border:2px solid #e2e8f0;cursor:pointer;overflow:hidden;flex-shrink:0;position:relative;}
.est-color input[type=color]{position:absolute;inset:-4px;width:calc(100%+8px);height:calc(100%+8px);border:none;cursor:pointer;opacity:0;}
.est-color-preview{position:absolute;inset:0;border-radius:5px;pointer-events:none;}
.est-name-inp{flex:1;border:none;background:transparent;font-size:13.5px;font-weight:600;color:#1e293b;outline:none;min-width:0;}
.est-name-inp::placeholder{color:#cbd5e1;}
.est-save-btn{height:28px;padding:0 10px;border:none;border-radius:7px;background:var(--pr);color:#fff;font-size:11.5px;font-weight:600;cursor:pointer;opacity:0;transition:opacity .15s;flex-shrink:0;}
.est-row:hover .est-save-btn,.est-row:focus-within .est-save-btn{opacity:1;}
.est-del-btn{width:28px;height:28px;border:none;border-radius:7px;background:transparent;cursor:pointer;display:flex;align-items:center;justify-content:center;color:#cbd5e1;transition:all .15s;flex-shrink:0;}
.est-del-btn:hover{background:#fee2e2;color:#ef4444;}
.est-del-btn svg{width:13px;height:13px;stroke:currentColor;fill:none;stroke-width:2;}
.btn-add-est{height:36px;width:100%;border:2px dashed #e2e8f0;border-radius:9px;background:transparent;font-size:13px;font-weight:500;color:#94a3b8;cursor:pointer;display:flex;align-items:center;justify-content:center;gap:6px;margin-top:8px;transition:all .18s;}
.btn-add-est:hover{border-color:var(--pr);color:var(--pr);background:rgba(var(--pr-rgb),.04);}

/* Tag management */
.tag-list{display:flex;flex-wrap:wrap;gap:7px;margin-bottom:8px;}
.tag-mgr-chip{display:inline-flex;align-items:center;gap:5px;padding:4px 10px 4px 12px;border-radius:20px;font-size:12.5px;font-weight:600;cursor:default;}
.tag-mgr-del{width:16px;height:16px;border-radius:50%;border:none;background:rgba(0,0,0,.12);color:inherit;cursor:pointer;display:flex;align-items:center;justify-content:center;font-size:10px;line-height:1;transition:background .15s;}
.tag-mgr-del:hover{background:rgba(0,0,0,.22);}
.tag-new-row{display:flex;gap:8px;}
.tag-new-inp{flex:1;height:36px;padding:0 12px;border:1px solid #e2e8f0;border-radius:9px;font-size:13px;color:#1e293b;background:#f8fafc;outline:none;font-family:inherit;transition:border-color .18s;}
.tag-new-inp:focus{border-color:var(--pr);background:#fff;}
.btn-add-tag{height:36px;padding:0 14px;border:none;border-radius:9px;background:var(--grad);color:#fff;font-size:13px;font-weight:600;cursor:pointer;flex-shrink:0;white-space:nowrap;}

/* ─── TOAST ─── */
.toast-wrap{position:fixed;bottom:22px;left:50%;transform:translateX(-50%);z-index:300;display:flex;flex-direction:column;align-items:center;gap:7px;pointer-events:none;}
.toast{background:#1e293b;color:#fff;border-radius:10px;padding:10px 16px;font-size:13px;font-weight:500;display:flex;align-items:center;gap:10px;box-shadow:0 8px 24px rgba(0,0,0,.2);pointer-events:all;animation:toastIn .22s cubic-bezier(.4,0,.2,1);max-width:380px;}
.toast.ok{border-left:3px solid #22c55e;}
.toast.err{border-left:3px solid #ef4444;background:#fff1f2;color:#1e293b;}
.toast.info{border-left:3px solid #3b82f6;}
.toast-undo{background:rgba(255,255,255,.16);border:none;color:#fff;padding:3px 10px;border-radius:6px;font-size:12px;font-weight:600;cursor:pointer;transition:background .15s;flex-shrink:0;}
.toast-undo:hover{background:rgba(255,255,255,.26);}
@keyframes toastIn{from{opacity:0;transform:translateY(14px);}to{opacity:1;transform:translateY(0);}}

/* ─── EDIT LEAD FORM ─── */
.lm-edit-form{display:none;border-bottom:1px solid #f0f4f8;}
.lm-edit-form.active{display:block;}
.lm-edit-grid{display:grid;grid-template-columns:1fr 1fr;gap:8px;padding:14px 20px 0;}
.lm-efield{display:flex;flex-direction:column;gap:3px;}
.lm-efield.full{grid-column:1/-1;}
.lm-elbl{font-size:10px;font-weight:700;color:#94a3b8;text-transform:uppercase;letter-spacing:.4px;}
.lm-einp{height:34px;padding:0 10px;border:1px solid #e2e8f0;border-radius:8px;font-size:13px;color:#1e293b;background:#f8fafc;outline:none;font-family:inherit;transition:border-color .18s;width:100%;}
.lm-einp:focus{border-color:var(--pr);background:#fff;box-shadow:0 0 0 3px rgba(var(--pr-rgb),.08);}
.lm-eta{height:56px;padding:8px 10px;resize:none;}
.lm-edit-footer{padding:10px 20px 14px;display:flex;gap:8px;}
.btn-edit-lead{height:30px;padding:0 13px;border:none;border-radius:8px;background:#fff;color:var(--pr);font-size:12.5px;font-weight:700;cursor:pointer;display:flex;align-items:center;gap:5px;transition:all .18s;flex-shrink:0;box-shadow:0 1px 5px rgba(0,0,0,.15);}
.btn-edit-lead:hover{background:#f0f4ff;box-shadow:0 2px 8px rgba(0,0,0,.18);transform:translateY(-1px);}
.btn-edit-lead svg{width:13px;height:13px;stroke:currentColor;fill:none;stroke-width:2;}
/* ─── VINCULAR PANEL ─── */
.lm-vinc-panel{display:none;margin-top:8px;background:#f8fafc;border:1px solid #e2e8f0;border-radius:10px;padding:10px;gap:8px;flex-direction:column;}
.lm-vinc-panel.open{display:flex;}
.lm-vinc-badge{font-size:11.5px;color:#16a34a;background:#f0fdf4;border:1px solid #bbf7d0;padding:3px 8px;border-radius:8px;display:none;}
.lm-vinc-badge.show{display:block;}
/* ─── DATE FILTERS ─── */
.pl-date-group{display:flex;align-items:center;gap:5px;background:#f8fafc;border:1px solid #e2e8f0;border-radius:8px;padding:0 10px;height:36px;flex-shrink:0;}
.pl-date-lbl{font-size:11.5px;font-weight:600;color:#64748b;white-space:nowrap;}
.pl-date-sep{font-size:12px;color:#94a3b8;flex-shrink:0;}
.pl-date{height:26px;padding:0 4px;border:none;background:transparent;font-size:12px;color:#475569;outline:none;cursor:pointer;width:108px;}
.pl-date:focus{color:var(--pr);}
.pl-date-clear{background:none;border:none;color:#94a3b8;cursor:pointer;font-size:13px;padding:0 2px;line-height:1;display:none;transition:color .15s;}
.pl-date-clear.show{display:block;}
.pl-date-clear:hover{color:#ef4444;}
...
@media(max-width:680px){
  .lm-box{flex-direction:column;height:92vh;}
  .lm-body{flex-direction:column;}
  .lm-left{width:100%;max-height:45%;border-right:none;border-bottom:1px solid #f0f4f8;}
  ...
  <div class="pl-toolbar">
    <div class="view-toggle">
      <button class="vbtn active" id="btnKanban" onclick="switchView('kanban')">
        <svg viewBox="0 0 24 24"><rect x="3" y="3" width="5" height="18" rx="1"/><rect x="10" y="3" width="5" height="18" rx="1"/><rect x="17" y="3" width="4" height="18" rx="1"/></svg>
        Tablero
      </button>
    ...
    <div class="pl-date-group">
      <svg viewBox="0 0 24 24" style="width:13px;height:13px;stroke:#94a3b8;fill:none;stroke-width:2;flex-shrink:0;"><rect x="3" y="4" width="18" height="18" rx="2"/><line x1="16" y1="2" x2="16" y2="6"/><line x1="8" y1="2" x2="8" y2="6"/><line x1="3" y1="10" x2="21" y2="10"/></svg>
    ...
    <div class="lm-body">
      ...
      <div class="lm-left">
        ...
        <div id="lmInfoSection">
          <!-- Destino -->
          <div class="lm-ig">
            <div class="lm-ig-icon"><svg viewBox="0 0 24 24"><path d="M21 10c0 7-9 13-9 13S3 17 3 10a9 9 0 0 1 18 0z"/><circle cx="12" cy="10" r="3"/></svg></div>
          ...
          <!-- Presupuesto -->
          <div class="lm-ig">
            <div class="lm-ig-icon"><svg viewBox="0 0 24 24"><line x1="12" y1="1" x2="12" y2="23"/><path d="M17 5H9.5a3.5 3.5 0 0 0 0 7h5a3.5 3.5 0 0 1 0 7H6"/></svg></div>
            <div class="lm-ig-content">
          ...
          <!-- Teléfono -->
          <div class="lm-ig">
            <div class="lm-ig-icon"><svg viewBox="0 0 24 24"><path d="M22 16.92v3a2 2 0 0 1-2.18 2 19.79 19.79 0 0 1-8.63-3.07A19.5 19.5 0 0 1 4.69 12 19.79 19.79 0 0 1 1.61 3.38 2 2 0 0 1 3.6 1h3a2 2 0 0 1 2 1.72c.127.96.361 1.903.7 2.81a2 2 0 0 1-.45 2.11L7.91 8.59a16 16 0 0 0 6 6l.95-.95a2 2 0 0 1 2.11-.45c.907.339 1.85.573 2.81.7A2 2 0 0 1 22 16.92z"/></svg></div>
            <div class="lm-ig-content">
          ...
          <!-- Viajeros -->
          <div class="lm-ig">
            <div class="lm-ig-icon"><svg viewBox="0 0 24 24"><path d="M17 21v-2a4 4 0 0 0-4-4H5a4 4 0 0 0-4 4v2"/><circle cx="9" cy="7" r="4"/><path d="M23 21v-2a4 4 0 0 0-3-3.87"/><path d="M16 3.13a4 4 0 0 1 0 7.75"/></svg></div>
            <div class="lm-ig-content">
          ...
          <!-- Fechas -->
          <div class="lm-ig">
            <div class="lm-ig-icon"><svg viewBox="0 0 24 24"><rect x="3" y="4" width="18" height="18" rx="2"/><line x1="16" y1="2" x2="16" y2="6"/><line x1="8" y1="2" x2="8" y2="6"/><line x1="3" y1="10" x2="21" y2="10"/></svg></div>
            <div class="lm-ig-content">
          ...
          <div class="lm-ig" id="lmIgNotasRow" style="display:none">
            <div class="lm-ig-icon"><svg viewBox="0 0 24 24"><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"/><polyline points="14 2 14 8 20 8"/><line x1="8" y1="13" x2="16" y2="13"/><line x1="8" y1="17" x2="13" y2="17"/></svg></div>
            <div class="lm-ig-content">
        ...
        <!-- Tags -->
        <div class="lm-ig">
          <div class="lm-ig-icon"><svg viewBox="0 0 24 24"><path d="M20.59 13.41l-7.17 7.17a2 2 0 0 1-2.83 0L2 12V2h10l8.59 8.59a2 2 0 0 1 0 2.82z"/><line x1="7" y1="7" x2="7.01" y2="7"/></svg></div>
          <div class="lm-ig-content">
            <span class="lm-ig-label">Tags <span style="font-weight:400;color:#cbd5e1;font-size:10px;">(clic)</span></span>
        ...
        <!-- Estado -->
        <div class="lm-ig">
          <div class="lm-ig-icon"><svg viewBox="0 0 24 24"><circle cx="12" cy="12" r="10"/><polyline points="12 6 12 12 16 14"/></svg></div>
          <div class="lm-ig-content">
        ...
        <!-- Asesor -->
        <div class="lm-ig">
          <div class="lm-ig-icon"><svg viewBox="0 0 24 24"><path d="M20 21v-2a4 4 0 0 0-4-4H8a4 4 0 0 0-4 4v2"/><circle cx="12" cy="7" r="4"/></svg></div>
          <div class="lm-ig-content">
        ...
        <div class="lm-edit-form" id="lmEditForm">
          <div class="lm-edit-grid">
            <div class="lm-efield"><label class="lm-elbl">Nombre *</label><input class="lm-einp" id="leNombre" type="text"></div>
            <div class="lm-efield"><label class="lm-elbl">Email *</label><input class="lm-einp" id="leEmail" type="email"></div>
            <div class="lm-efield full"><label class="lm-elbl">Teléfono</label><input class="lm-einp" id="leTel" type="text" placeholder="+57…"></div>
            <div class="lm-efield full"><label class="lm-elbl">Destino *</label><input class="lm-einp" id="leDestino" type="text"></div>
            <div class="lm-efield"><label class="lm-elbl">Fecha salida</label><input class="lm-einp" id="leFecSal" type="date"></div>
            <div class="lm-efield"><label class="lm-elbl">Fecha regreso</label><input class="lm-einp" id="leFecLleg" type="date"></div>
            <div class="lm-efield"><label class="lm-elbl">Viajeros</label><input class="lm-einp" id="leViajeros" type="number" min="1"></div>
            <div class="lm-efield"><label class="lm-elbl">Presupuesto (USD)</label><input class="lm-einp" id="leBudget" type="number" min="0"></div>
            <div class="lm-efield full"><label class="lm-elbl">Origen</label><input class="lm-einp" id="leSource" type="text" placeholder="WhatsApp, web…"></div>
            <div class="lm-efield full"><label class="lm-elbl">Descripción</label><textarea class="lm-einp lm-eta" id="leDesc"></textarea></div>
          </div>
          <div class="lm-edit-footer">
            <button class="btn-submit" style="flex:1;height:36px;font-size:13px;" id="btnSaveEdit" onclick="saveLeadEdit()">Guardar cambios</button>
            <button class="btn-cancel" style="height:36px;font-size:13px;" onclick="cancelLeadEdit()">Cancelar</button>
          </div>
        </div>
      </div>
      <!-- RIGHT: Chat panel -->
      <div class="lm-right" id="lmRight">
        ...
        <div class="lm-composer" id="lmComposer">
          ...
          <div class="lm-fmt-bar">
            <button class="lm-fmt-btn" id="lmFmtBold" title="Negrita" onclick="lmApplyFormat('bold')"><b>B</b></button>
            <button class="lm-fmt-btn" id="lmFmtItalic" title="Cursiva" onclick="lmApplyFormat('italic')"><i style="font-style:italic">I</i></button>
            <button class="lm-fmt-btn" id="lmFmtUnder" title="Subrayado" onclick="lmApplyFormat('underline')" style="text-decoration:underline;font-weight:600">U</button>
            <div class="lm-fmt-divider"></div>
            <button class="lm-fmt-btn" id="lmFmtStrike" title="Tachado" onclick="lmApplyFormat('strikethrough')" style="text-decoration:line-through;font-weight:600">S</button>
          </div>
          <!-- Editor -->
          <div class="lm-editor-wrap">
            <div id="lmEditor" class="lm-editor" contenteditable="true" data-placeholder="Escribe un mensaje al cliente..."></div>
          </div>
          <!-- Adjuntos preview -->
          ...
          <div class="lm-action-bar">
            <div class="lm-act-left">
              <button class="lm-act-btn" title="Adjuntar archivo" onclick="document.getElementById('lmFileInput').click()">
                <svg viewBox="0 0 24 24"><path d="M21.44 11.05l-9.19 9.19a6 6 0 0 1-8.49-8.49l9.19-9.19a4 4 0 0 1 5.66 5.66L9.41 17.4a2 2 0 0 1-2.83-2.83l8.49-8.48"/></svg>
              </button>
...
<div class="lm-tpl-overlay" id="lmTplOverlay" onclick="lmCerrarTemplates(event)">
  <div class="lm-tpl-modal">
    <div class="lm-tpl-hd">
      <h2>Templates</h2>
      <input type="text" class="lm-tpl-search" id="lmTplSearch" placeholder="Buscar template..." oninput="lmFiltrarTemplates()">
...
<div id="vincModal" style="position:fixed;inset:0;background:rgba(15,23,42,.65);z-index:1200;display:none;align-items:center;justify-content:center;padding:16px;">
  <div style="background:#fff;border-radius:18px;width:min(520px,94vw);display:flex;flex-direction:column;overflow:hidden;box-shadow:0 24px 60px rgba(0,0,0,.28);animation:mdIn .22s cubic-bezier(.4,0,.2,1);">
    <!-- Header -->
    <div style="padding:18px 22px 14px;border-bottom:1px solid #f1f5f9;display:flex;align-items:flex-start;justify-content:space-between;gap:12px;flex-shrink:0;">
      <div>
        <div id="vincTitle" style="font-size:16px;font-weight:700;color:#0f172a;">Vincular itinerario</div>
        <div id="vincSubtitle" style="font-size:12px;color:#64748b;margin-top:3px;">¿Cómo quieres asociar el itinerario a este lead?</div>
      </div>
      <button class="lm-close" onclick="closeVincModal()"><svg viewBox="0 0 24 24"><line x1="18" y1="6" x2="6" y2="18"/><line x1="6" y1="6" x2="18" y2="18"/></svg></button>
    </div>
    ...
    <div id="vincViewMain" style="padding:16px 22px 20px;display:flex;flex-direction:column;gap:8px;">
      <button class="vinc-opt" onclick="vincSelectOption('existente')">
        <div class="vinc-opt-ico"><svg viewBox="0 0 24 24"><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"/><polyline points="14 2 14 8 20 8"/><line x1="16" y1="13" x2="8" y2="13"/><line x1="16" y1="17" x2="8" y2="17"/></svg></div>
        <div><div class="vinc-opt-title">Seleccionar programa existente</div><div class="vinc-opt-desc">Vincula un programa que ya tienes creado</div></div>
        <svg class="vinc-opt-arr" viewBox="0 0 24 24"><polyline points="9 18 15 12 9 6"/></svg>
      </button>
      <button class="vinc-opt" onclick="vincSelectOption('scratch')">
        <div class="vinc-opt-ico"><svg viewBox="0 0 24 24"><line x1="12" y1="5" x2="12" y2="19"/><line x1="5" y1="12" x2="19" y2="12"/></svg></div>
        <div><div class="vinc-opt-title">Crear programa desde cero</div><div class="vinc-opt-desc">Diseña un nuevo itinerario personalizado</div></div>
        <svg class="vinc-opt-arr" viewBox="0 0 24 24"><polyline points="9 18 15 12 9 6"/></svg>
      </button>
      <button class="vinc-opt" onclick="vincSelectOption('clonar')">
        <div class="vinc-opt-ico"><svg viewBox="0 0 24 24"><rect x="9" y="9" width="13" height="13" rx="2" ry="2"/><path d="M5 15H4a2 2 0 0 1-2-2V4a2 2 0 0 1 2-2h9a2 2 0 0 1 2 2v1"/></svg></div>
        <div><div class="vinc-opt-title">Crear a partir de otro programa</div><div class="vinc-opt-desc">Copia un programa existente como punto de partida</div></div>
        <svg class="vinc-opt-arr" viewBox="0 0 24 24"><polyline points="9 18 15 12 9 6"/></svg>
      </button>
      <button class="vinc-opt" onclick="vincSelectOption('plantilla')">
        <div class="vinc-opt-ico"><svg viewBox="0 0 24 24"><rect x="3" y="3" width="18" height="18" rx="2" ry="2"/><line x1="3" y1="9" x2="21" y2="9"/><line x1="9" y1="3" x2="9" y2="21"/></svg></div>
        <div><div class="vinc-opt-title">Crear a partir de plantilla</div><div class="vinc-opt-desc">Usa una de tus plantillas de programa guardadas</div></div>
        <svg class="vinc-opt-arr" viewBox="0 0 24 24"><polyline points="9 18 15 12 9 6"/></svg>
      </button>
    </div>
    <!-- Sub-vista: Existente -->
    <div id="vincViewExistente" style="padding:14px 22px 4px;display:none;flex-direction:column;gap:8px;">
      <div class="vinc-psearch">
        <svg viewBox="0 0 24 24"><circle cx="11" cy="11" r="8"/><path d="m21 21-4.35-4.35"/></svg>
        <input type="text" class="vinc-pinput" id="vincProgSearch" placeholder="Buscar programa..." oninput="_renderVincPicker('existente')">
      </div>
      <div class="vinc-pgrid" id="vincProgGrid"></div>
    </div>
    <!-- Sub-vista: Clonar programa -->
    <div id="vincViewClonar" style="padding:8px 22px 4px;display:none;flex-direction:column;max-height:280px;overflow-y:auto;">
    ...
    <div id="vincViewPlantilla" style="padding:14px 22px 4px;display:none;flex-direction:column;gap:8px;">
      <div class="vinc-psearch">
        <svg viewBox="0 0 24 24"><circle cx="11" cy="11" r="8"/><path d="m21 21-4.35-4.35"/></svg>
        <input type="text" class="vinc-pinput" id="vincPlantSearch" placeholder="Buscar plantilla..." oninput="_renderVincPicker('plantilla')">
      </div>
      <div class="vinc-pgrid" id="vincPlantGrid"></div>
    </div>
    <!-- Footer sub-vistas -->
    <div id="vincFooter" style="padding:12px 22px 18px;display:none;flex-direction:row;justify-content:space-between;align-items:center;gap:9px;flex-shrink:0;border-top:1px solid #f1f5f9;margin-top:8px;">
      <button class="btn-cancel" onclick="vincBack()">← Atrás</button>
      <button id="vincConfirmBtn" class="btn-submit" style="height:38px;padding:0 22px;" onclick="confirmarVincular()">Vincular</button>
    </div>
  </div>
</div>
...
<div class="nl-overlay" id="newLeadModal">
  <div class="nl-box">
    <div class="nl-hd">
      ...
      <button class="nl-close" onclick="closeNewLead()"><svg viewBox="0 0 24 24"><line x1="18" y1="6" x2="6" y2="18"/><line x1="6" y1="6" x2="18" y2="18"/></svg></button>
    </div>
    <div class="nl-body">
      <div class="f-grid">
        <div class="f-field">
        ...
        <div class="f-field">
          ...
          <select class="f-sel" id="fEstado"><option value="">Seleccionar…</option></select>
        </div>
        <?php if ($isAdmin): ?>
        <div class="f-field">
          <label class="f-lbl">Asesor</label>
          <select class="f-sel" id="fAsesor"><option value="">Sin asignar</option></select>
        </div>
        <?php endif; ?>
        <div class="f-field full">
...
<div class="cfg-overlay" id="cfgModal">
  <div class="cfg-box">
    <div class="cfg-hd">
      ...
      <button class="nl-close" onclick="closeConfig()"><svg viewBox="0 0 24 24"><line x1="18" y1="6" x2="6" y2="18"/><line x1="6" y1="6" x2="18" y2="18"/></svg></button>
    </div>
    <div class="cfg-body">
      ...
      <div class="tag-new-row">
        <input class="tag-new-inp" id="tagNewInp" type="text" placeholder="Nombre del tag…" onkeydown="if(event.key==='Enter')addTag()">
