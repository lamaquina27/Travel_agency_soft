...
<body>
    ...
    <div class="main-content" id="mainContent">
        <div class="tabs-container">
            ...
            <div class="filters-section">
                <input type="text" class="search-input" placeholder="Buscar por título, descripción, ubicación..."
                    id="searchInput">
    ...
    <script>
        ...
        /**
         * Normaliza URLs de imágenes: convierte URLs absolutas del hosting
         * en rutas relativas prefijadascon APP_URL local.
         */
    ...
    `;
        ...
        function createResourceCard(item) {
            ...
            if (currentTab === 'dias' || currentTab === 'actividades') {
                const images = [];
                if (item.imagen1) images.push(normalizarImagenUrl(item.imagen1));
                if (item.imagen2) images.push(normalizarImagenUrl(item.imagen2));
                if (item.imagen3) images.push(normalizarImagenUrl(item.imagen3));
        ...
        `;
                    ...
                    // Para alojamientos
                    const primaryImageRaw = getPrimaryImage(item, currentTab);
                    const primaryImage = primaryImageRaw ? normalizarImagenUrl(primaryImageRaw) : null;
                    imageHTML = `
            ...
            imageFields.forEach(field => {
                if (resource[field]) {
                    ...
                    if (input) {
                        ...
                        preview.src = normalizarImagenUrl(resource[field]);
                        ...
                        preview.addEventListener('click', function () {
                            showImageModal(normalizarImagenUrl(resource[field]), resource.titulo || resource.nombre || 'Imagen');
                        });
    ...
    `;
        ...
        function getResourceImages(resource, type) {
            ...
            switch (type) {
                case 'dias':
                case 'actividades':
                    if (resource.imagen1) images.push(normalizarImagenUrl(resource.imagen1));
                    if (resource.imagen2) images.push(normalizarImagenUrl(resource.imagen2));
                    if (resource.imagen3) images.push(normalizarImagenUrl(resource.imagen3));
                    break;
                case 'alojamientos':
                    if (resource.imagen) images.push(normalizarImagenUrl(resource.imagen));
                    break;
            }
